...
valid_data <- tibble::tibble(
  ...
  provider_type = factor(
    sample(
      c("Surgery/Trauma", "Emergency Medicine"),
      size = n_patients,
      replace = TRUE
    )
  ),
...
Here we can answer the question: "In what proportion of trauma cases was the trauma surgeon present in ED within 15 minutes (1a) and 5 minutes (1d) of patient arrival?"
...
[^1]: By default, the `seqic_indicator_*()` functions will also give the numerator and denominator, as well. I just kicked them out for space.

```{r calc-seqic-1, echo=FALSE, results='markup'}
calc_indicator_1 <- indicator_1 |>
  dplyr::mutate(
    dplyr::across(
      -1,
      ~ ifelse(
...
## `traumar`: Risk-Adjusted Mortality

Functions assume that you have a variable representing predicted probabilities of survival for patients in your dataset [^2].

[^2]: Most trauma registry software will utilize the Trauma and Injury Severity Score (TRISS) method from the Major Trauma Outcomes Study.

-   Use coefficients from that logistic regression model to calculate predictions based on Revised Trauma Score, Injury Severity Score, Age, mechanism of injury.
-   `traumar::probability_of_survival()` calculates the predicted probabilities for you if you do not have it.

------------------------------------------------------------------------

## `traumar`: Risk-Adjusted Mortality

-   Champion et al. (1990; MTOS) utilized linear bins (i.e. 0 - 0.1, 0.1 - 0.2, etc.).
-   Empirically, patients distributed heavily in \> 80% survival probability.
-   Napoli et al. (2017): use a nonlinear binning process [^3]
    -   Stratify patients into relevant groups
    -   Enable examination of outcome patterns across groups

[^3]: From the data on patients in these bins, we calculate the relative mortality metric.
...
::: notes
-   `threshold_1` and `threshold_2`
    -   These values define break points in the ordered survival probabilities where the binning logic changes.
    -   `threshold_1` marks the lower bound of the "high survival probability" region.
    -   `threshold_2` marks the upper bound of that region (very high survival probabilities).

The function creates finer bins between these thresholds and coarser bins outside them, reflecting the approach used in the cited literature to improve resolution where outcome discrimination is most clinically relevant.

-   `divisor1` and `divisor2`
    -   These control the bin widths within the ranges defined by the thresholds.
    -   `divisor1` determines how many bins are created in the range from the lowest Ps value up to threshold_1.
    -   `divisor2` determines how many bins are created between threshold_1 and threshold_2.

Smaller divisor values produce fewer, broader bins; larger values produce narrower, more granular bins.
:::
...
epatient_15 <- as.integer(floor(
  as.numeric(difftime(incident_date, patient_dob, units = "days")) / 365.25
))
...
evitals_12 <- dplyr::if_else(
  runif(n) < 0.2,
  sample(60:89, n, replace = TRUE),
  evitals_12
)
...
We hope to add data on missing values in variables used for calculations to the `filter_process` output, soon!
...
-   `nemsqar`[^4]: First statewide report on all 21 NEMSQA measures in Iowa
    -   Project and examples: https://github.com/bemts-hhs/NEMSQA-Report-2025
-   Reproducible, interpretable, fast

[^4]: Similar trauma/EMS bureaus in Oregon and Connecticut are using `nemsqar`, along with the quality managers at Dallas Fire and Washington D.C. Fire
